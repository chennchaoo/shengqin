...
function action_send_mobile_code ()
{
	...
	//$content = sprintf($_LANG['mobile_code_template'], $GLOBALS['_CFG']['shop_name'], $mobile_code, $GLOBALS['_CFG']['shop_name']);
        $content = "您的注册校验码为".$mobile_code."请勿泄露";
	/* 发送激活验证邮件 */
	// $result = true;
 ...
 * @desc 检查是否存在此推荐人员账号
 */
function action_check_referral_exist ()
{
	$_LANG = $GLOBALS['_LANG'];
	$_CFG = $GLOBALS['_CFG'];
	$smarty = $GLOBALS['smarty'];
	...
	if($user->check_referral($mobile_phone))
	{
		echo 'true';
	}
	else
...
function action_register ()
{
	...
	else
	{
		...
		if($register_type == "email")
		{
			...
			/*邮箱验证去掉验证码随便输入，可临时注册if(empty($email_code))
			{
		...
		else if($register_type == "mobile")
		{
			...
			/* 手机注册 */
			$result = register_by_mobile($username, $password, $mobile_phone, $other, $referral);
