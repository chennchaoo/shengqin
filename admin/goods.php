...
//-------------------------------
   if(isset($goods['region_id'])&& !empty($goods['region_id'])){
        $area = get_city($goods['region_id']);
        if($area){
            $city = get_city($area['parent_id']);
            $goods['city'] = $area['parent_id'];
            if($city){
                $province = get_city($city['parent_id']);
                $goods['province'] = $city['parent_id'];
            }
        }
    }
    $province_list = get_regions(1, 1);
    $city_list = get_regions(2, $city['parent_id']);
    $district_list = get_regions(3, $area['parent_id']);
...
{
	if(isset($_REQUEST[goods_id])){
		$sql = "SELECT * FROM ecs_goods  WHERE goods_id = '$_REQUEST[goods_id]'";
		$ggood = $db->getRow($sql);		
	}
	...
	//是否上传证书
    if (isset($_FILES['cert_img']))
    {
        // 上传了，直接使用，原始大小
        $cert_img = $image->upload_image($_FILES['cert_img']);
        if ($cert_img === false)
        {
            $cert_img = $ggood['cert_img'];
        }
    }
    ...
    if (isset($_FILES['resume']) && !empty($_FILES['resume']['name'])){
        ...
        $ext=explode(".", $resume_name);
        $tt = time().rand(100,999);
        $r_name = $tt.".".$ext['1'];
        $resume = 'resume/'.$r_name;
        move_uploaded_file($tmp, $path.$r_name);
    }*/

    /* 处理商品数据 */
    $shop_price = !empty($_POST['shop_price']) ? $_POST['shop_price'] : 0;
    $market_price = !empty($_POST['market_price']) ? $_POST['market_price'] : 0;
    $promote_price = !empty($_POST['promote_price']) ? $_POST['promote_price']: 0;
    ...
    $cert_no = isset($_POST['cert_no'])? $_POST['cert_no']:$ggood['cert_no'];
	...
	$cert_img = (empty($cert_img))? $ggood['cert_img'] : $cert_img;
    ...
    $is_product_type = !empty($_POST['is_product_type']) ? $_POST['is_product_type'] : '1';
    $experience = !empty($_POST['experience']) ? $_POST['experience'] : '';
    $charge_price = !empty($_POST['charge_price']) ? $_POST['charge_price'] : '';
    $member_price = !empty($_POST['member_price']) ? $_POST['member_price'] : '';
    //$province = !empty($_POST['province']) ? $_POST['province'] : '';
    //$city = !empty($_POST['city']) ? $_POST['city'] : '';
    //$region_id = !empty($_POST['region_id']) ? $_POST['region_id'] : '';
    $stars = !empty($_POST['stars']) ? trim($_POST['stars']) : '';
    $service_type = !empty($_POST['service_type']) ? $_POST['service_type'] : '';
    $gender = !empty($_POST['gender']) ? $_POST['gender'] : '';
    $age = !empty($_POST['age']) ? $_POST['age'] : '';
    $position_state = !empty($_POST['position_state']) ? $_POST['position_state'] : '';
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id, cost_price, is_product_type, experience, charge_price, member_price, stars, service_type, gender, age, position_state,cert_img,cert_no)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id', '$cost_price',$is_product_type , '$experience', '$charge_price', '$member_price', '$stars', '$service_type', '$gender', '$age', '$position_state','$cert_img','$cert_no')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral, cost_price,is_product_type , experience, charge_price, member_price, stars, service_type, gender, age, position_state,cert_img,cert_no)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote', '$zhekou', '$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$is_buy','$buymax','$buymax_start_date','$buymax_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral', '$cost_price', $is_product_type, '$experience', '$charge_price', '$member_price', '$stars', '$service_type', '$gender', '$age', '$position_state','$cert_img','$cert_no')";
        }
    }
    else
    {
        /* 如果有上传图片，删除原来的商品图 */
        $sql = "SELECT goods_thumb, goods_img, original_img, resume " .
                    " FROM " . $ecs->table('goods') .
                    " WHERE goods_id = '$_REQUEST[goods_id]'";
        $row = $db->getRow($sql);
        ...
        /*if(!empty($resume) && !empty($row['resume'])){
            @unlink(ROOT_PATH . $row['resume']);
        }*/
		...
		"zhekou = '$zhekou', " .
                ...
                "cert_img = '$cert_img',".
                "cert_no = '$cert_no',".
                "promote_end_date = '$promote_end_date', ";//gender, age, position_state
