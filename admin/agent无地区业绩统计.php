...
elseif($_REQUEST['act']=='insert'){
    ...
    $sql = 'INSERT INTO ' . $ecs->table('agent') . ' (`name`, `gender`, `tel`, `birthday`, `join_time`, `agent_products`, `region_id`, `remark`, `add_time`) '.
            " VALUES ('$name','$gender','$tel','$birthday','$join_time','$agent_products','$region_id','$remark','$add_time')";
    $db->query($sql);
...
elseif($_REQUEST['act']=='edit'){
    ...
    if(isset($info['region_id'])&& !empty($info['region_id'])){
        ...
        if($area){
            $city = get_city($area['parent_id']);
            $info['city'] = $area['parent_id'];
            if($city){
                $province = get_city($city['parent_id']);
                $info['province'] = $city['parent_id'];
            }
        }
    }
...
elseif($_REQUEST['act']=='update'){
    ...
    $remark         = $_POST['remark'];
    $add_time       = time();
    $sql = "update " . $ecs->table('agent') . " set `name`='$name',`gender`='$gender',`tel`='$tel',`birthday`='$birthday',`join_time`='$join_time',`agent_products`='$agent_products',`region_id`='$region_id',`remark`='$remark',`add_time`='$add_time' where id = " .  intval($id);
    //echo $sql;exit;
    $db->query($sql);
