...
function action_insert ()
{
	...
	$parent_id= empty($_POST['parent_id']) ? '' : trim($_POST['parent_id']);
...
function action_referral(){
    ...
    $smarty->assign('ur_here', '我推荐的人');
        
    $sql = "SELECT user_name FROM " . $ecs->table('users') . "  WHERE parent_id=".  intval($_GET['id']);
    //echo $sql;exit;
    $data = $db->getAll($sql);
