...
else
{
   ...
   /* fulltext_search_add_END_www.68ecshop.com */
    ...
    $sql   = "SELECT COUNT(*) FROM " .$ecs->table('goods'). " AS g ".
        "WHERE g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.city=$city AND g.is_virtual=0  $attr_in $categories ".
    ...
    /* 查询商品 */
    $sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.click_count, g.goods_number, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, g.is_product_type, g.experience, g.charge_price, g.member_price, g.stars, g.position_state,  ".
    ...
    if($_CFG['fulltext_search'] == '1'){
        ...
        if(empty($ids)) $ids = '-1';
        $sql   = "SELECT COUNT(*) FROM " .$ecs->table('goods'). " AS g ".
        "WHERE g.is_delete = 0 AND g.is_on_sale = 1 AND (g.region_id in ($city)) AND g.is_alone_sale = 1 $attr_in $categories ".
    ...
    if ($page > $max_page)
    {
        ...
        $sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.click_count, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, g.is_product_type, g.experience, g.charge_price, g.member_price, g.stars, g.position_state, ".
                    "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                    "g.promote_price, g.promote_start_date, g.promote_end_date, g.goods_thumb, g.goods_img, g.goods_brief, g.goods_type ".
                "FROM " .$ecs->table('goods'). " AS g ".
                "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
