...
function get_region_id()
{
    if(isset($_COOKIE['city'])&&!empty($_COOKIE['city'])){
        ...
        $area = getRegion();
        $data = (array)json_decode($area);
        $city = $data['data']->city;
        $str = mb_substr($city,-1,1,'utf-8');
        $string = '';
        if($str=='市'){
            $string = mb_substr($city,0,-1,'utf-8');
        }else{
            $string = $city;
        }
        $sql = 'SELECT region_id,region_name FROM ' . $GLOBALS['ecs']->table('region') .
                " WHERE region_type = 2 and region_name like '%$string%'";
        $citys = $GLOBALS['db']->GetRow($sql);
        setcookie("city", $citys['region_id']);
        setcookie("city_name", $citys['region_name']);
        $city = $citys['region_id'];
        //echo $city;
    }
    return $city;
}
...
/**
 * 根据城市id取得属于当前城市的所有地区的id
 */
function get_city_id(){
    $parent = get_region_id();
    $arr = get_regions(3, $parent);
    $str = '';
    foreach ($arr as $val){
        $str .= $val['region_id'].',';
    }
...
function load_config()
{
    ...
    if ($data === false)
    {
		...
		$shop_province = '';
		$shop_city = '';
        if(!empty($arr['shop_province'])){
			$shop_province = $GLOBALS['db']->getOne("SELECT region_name FROM " . $GLOBALS['ecs']->table('region') . " WHERE region_id=".$arr['shop_province']);
		}
		if(!empty($arr['shop_city'])){
			$shop_city = $GLOBALS['db']->getOne("SELECT region_name FROM " . $GLOBALS['ecs']->table('region') . " WHERE region_id=".$arr['shop_city']);
		}
		
        $arr['shop_site'] = $shop_province.$shop_city;
...
//        $ww = $GLOBALS['db']->getAll("SELECT cus_no FROM " . $GLOBALS['ecs']->table('chat_third_customer') . " WHERE is_master = 1 AND cus_type = 1");
        if($arr['qq']){
            $arr['qq'] = explode(',', $arr['qq']);
        }
        if($arr['ww']){
            $arr['ww'] = explode(',', $arr['ww']);
        }
...
function get_brands_list($cat = 0, $app = 'brand', $num=18)
{
    ...
    $sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, b.brand_desc, COUNT(*) AS goods_num, IF(b.brand_logo > '', '1', '0') AS tag ".
            ...
            "GROUP BY b.brand_id HAVING goods_num > 0 ORDER BY tag DESC, b.sort_order ASC limit $num";
...
function get_dir($type, $id)
{
  ...
  * @return string
  */
function getIP() //本机ip 221.2.64.18
{
    global $ip;
