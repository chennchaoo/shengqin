 ...
 * @param    string   $name_account 开户人
 * @param    int      $card_number 银行卡号
 * @return  int
 */
function insert_user_account($surplus, $amount, $name_account='',$card_number='')
{
    $sql = 'INSERT INTO ' .$GLOBALS['ecs']->table('user_account').
           ' (user_id, admin_user, amount, add_time, paid_time, admin_note, user_note, process_type, payment, is_paid, name_account, card_number)'.
            " VALUES ('$surplus[user_id]', '', '$amount', '".gmtime()."', 0, '', '$surplus[user_note]', '$surplus[process_type]', '$surplus[payment]', 0,'$name_account','$card_number')";
    $GLOBALS['db']->query($sql);
