...
class integrate
{
	...
	function check_mobile_phone ($mobile_phone)
	{
         ...
         * @return boolean
         */
        function check_referral ($user_name)
	{
		if(! empty($user_name))
		{
			/* 检查是否存在此会员 */
			$sql = "SELECT " . $this->field_id . " FROM " . $this->table($this->user_table) . " WHERE " . $this->field_mobile_phone . " = '$user_name' or " . $this->field_name . "= '$user_name'";
			if($this->db->getOne($sql, true) > 0)
			{
				$this->error = ERR_MOBILE_PHONE_EXISTS;
				return true;
