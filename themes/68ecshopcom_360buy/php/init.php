...
/**
 * @desc 根据顶级栏目id获取所属子栏目
 * @param type $pid 顶级栏目id
 * @return array
 */
function get_category($pid,$limit=5)
{
        $sql = "select cat_id,cat_name from ".$GLOBALS['ecs']->table( "category" )." where is_show=1 and parent_id = ".$pid." limit $limit";
        $data = array();
        $data = $GLOBALS['db']->getAll( $sql );
        if(!empty($data)){
            foreach ($data as $key=>$val){
                $data[$key]['url'] = build_uri('category', array('cid' => $val['cat_id']), $val['cat_name']);
            }
        }
        return $data;
}
...
/**
 * @desc 获取发布信息
 * @param int $type_id 1我要做家政；2我要找家政；3房屋出租；4房源求租；5房源求购；6房源出售
 * @param int $num
 * @return array
 */
function get_release_information($type_id,$num=10){
    $sql = "SELECT * FROM ".$GLOBALS['ecs']->table( "release_information" )." WHERE type_id = ".$type_id." order by refresh_time desc,add_time desc limit ".$num;
    ...
    foreach ( $arr as $key => $row ) {
        ...
        if($type_id==1 || $type_id==2){
            ...
            $row['surname'] = mb_substr($row['name'],0,10,'utf-8');//获取姓
            $row['chenghu'] = $chenghu;
            $city = get_city($row['region_id']);
            $row['district'] = $city['region_name']; //区
            $row['last_current'] = time_tran($row['refresh_time']);
            $row['info'] = sub_str($row['content'],36);
        }
        $row['url'] = 'release.php?act=release_view&id='.$row['id'];
        $res[] = $row;
    }
    return $res;
}
