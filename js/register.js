...
function checkReferral(person, callback) {
	var submit_disabled = false;
	...
	if (typeof (person) == 'object') {
		personObj = $(person);
		person = personObj.val();
	}

        if(!Utils.isEmpty(person)){
            $.post('register.php?act=check_referral_exist', {
		mobile_phone: person
                }, function(result) {
                        if (result == 'false') {
                                document.getElementById('referral_person_notice').innerHTML = '- 不存在此会员';
                                document.getElementById('referral_person_notice').style.color = '#900';
                                document.forms['formUser'].elements['Submit'].disabled = '';
                                ...
                                document.getElementById('referral_person_notice').innerHTML = '';
                                document.getElementById('referral_person_notice').style.color = '#900';
                                document.forms['formUser'].elements['Submit'].disabled = 'disabled';
...
//	if (mobileObj == null) {
//		checkMobilePhoneExist(person, callback);
//	} else {
//		checkMobilePhoneExist(personObj, callback);
//	}
}
