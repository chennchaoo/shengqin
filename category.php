...
//echo '<pre>';print_r(category_get_goods($children, $brand, $price_min, $price_max, $ext, $size, $page, $sort, $order));exit;
if ((DEBUG_MODE & 2) != 2)
{
    $smarty->caching = true;
}
...
//echo '<pre>';print_r(get_flash_img(get_parent($cat_id)));exit;
$parent_id = get_parent($cat_id);
$smarty->assign('flash_img_list',get_flash_img($parent_id));
...
else
{
	...
	$default_sort_order_method = $_CFG['sort_order_method'] == '0' ? 'DESC' : 'ASC';
        $default_sort_order_method = 'DESC';
	$default_sort_order_type   = $_CFG['sort_order_type'] == '0' ? 'goods_id' : ($_CFG['sort_order_type'] == '1' ? 'shop_price' : 'last_update');
	$default_sort_order_type   = $_CFG['sort_order_type'] == '0' ? 'goods_id' : ($_CFG['sort_order_type'] == '1' ? 'shop_price' : 'last_update');
        $default_sort_order_type = 'salenum';
	...
	if (!$smarty->is_cached($index_dwt_files, $cache_id))
	{
		...
		$smarty->assign('parent',$parent_id);
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    $sql = "SELECT g.goods_id, g.service_type,g.goods_name, g.goods_name_style, g.click_count, g.goods_number, g.market_price, " .
		   ...
		   " g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, g.is_product_type,g.experience,g.charge_price,g.member_price,g.stars,g.position_state " .
	...
	if ($sort=='salenum')
	{
		$sql = "SELECT IFNULL(o.num,0) AS salenum, g.goods_id,g.service_type, g.goods_name, g.click_count, g.goods_number, g.goods_name_style, " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['is_product_type'] = $row['is_product_type'];
        $arr[$row['goods_id']]['experience']      = $row['experience'];
        $arr[$row['goods_id']]['charge_price']    = $row['charge_price'];
        $arr[$row['goods_id']]['member_price']    = $row['member_price'];
        $arr[$row['goods_id']]['stars']    = $row['stars'];
        $arr[$row['goods_id']]['position_state']    = $row['position_state'];
		...
		$arr[$row['goods_id']]['service_type']	   = $row['service_type'];
...
{ 
    ...
    { 
        $fimg_list[$i]=$row_fimg; 
        $fimg_list[$i]['img_url']=  DATA_DIR .'/catflashimg/'.$row_fimg['img_url']; 
        $fimg_list[$i]['img_link']= $row_fimg['href_url']; 
        $fimg_list[$i]['img_title']=trim($row_fimg['img_title']); 
        $fimg_list[$i]['img_desc']=trim($row_fimg['img_desc']); 
        $i++;
    } 
    return $fimg_list; 
}
