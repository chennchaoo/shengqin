...
function action_do_housewifery(){
	...
	$id = intval($_POST['id']);
	if($id){
		$sql = 'update ' . $ecs->table('release_information') . 
		" set name = '$name',gender='$gender',age='$age',tel='$tel',
		title='$title',work_time='$work_time',education='$education',
		price='$price',content = '$content',region_id = '$region_id',
		address = '$address',refresh_time = '$refresh_time' ,cert_no = '$cert_no'
		where id=".$id;
		$query = $db->query($sql);
		if($query > 0) {
			...
			show_message('操作失败！', '返回上一页，重新填写信息', 'release.php?act=default&id='.$id);
                ...
                $img = empty($_POST['zhengshu'])?'':$_POST['zhengshu'];
		$sql = "INSERT INTO " . $ecs->table('release_information') . "(user_id, name, gender, age, tel, title, work_time, education, type_id, price, content, region_id, address, add_time, refresh_time,cert_no)" .
				"VALUES ( '$user_id', '$name', '$gender', '$age', '$tel', '$title', '$work_time', '$education', '$type_id', '$price', '$content', '$region_id', '$address', '$add_time','$refresh_time','$cert_no')";
		//echo $sql;exit;
		$db->query($sql);
		$id = $db->insert_id();
	...
	if($id){
		...
		if($query > 0) {
			...
			show_message('操作失败！', '返回上一页，重新填写信息', 'release.php?act=rental&id='.$id);
                ...
                $img = empty($_POST['zufang'])?'':$_POST['zufang'];
...
function action_do_wanted(){
	...
	if($id){
		...
		$sql = "INSERT INTO " . $ecs->table('release_information') . 
				"(user_id, name, tel, title, type_id, price, content, region_id, address, into_time, add_time, refresh_time, huxingshi)" .
...
function action_do_chushou(){
	...
	$id = intval($_POST['id']);
	if($id){
		$sql = 'update ' . $ecs->table('release_information') . " set name = '$name',tel='$tel',title='$title',price='$price',content='$content',region_id='$region_id',address='$address',refresh_time='$refresh_time',district_name='$district_name',huxingshi = '$huxingshi',huxingting = '$huxingting',huxingwei = '$huxingwei',acreage = '$acreage' where id=".$id;
		$query = $db->query($sql);
		if($query > 0) {
			...
			show_message('操作失败！', '返回上一页，重新填写信息', 'release.php?act=chushou&id='.$id);
                ...
                $img = empty($_POST['chushou'])?'':$_POST['chushou'];
		$sql = "INSERT INTO " . $ecs->table('release_information') . 
				"(user_id, name, tel, title, type_id, price, content, region_id, address, add_time, refresh_time, district_name, huxingshi, huxingting, huxingwei, acreage)" .
				"VALUES ( '$user_id', '$name', '$tel', '$title', '$type_id', '$price', '$content', '$region_id', $address, '$add_time', '$refresh_time', '$district_name', '$huxingshi', '$huxingting', '$huxingwei', '$acreage')";
		//echo $sql;exit;
		$db->query($sql);
		$id = $db->insert_id();
...
function release_info($id){
    ...
    $info = $db->getRow($sql);
    if($info['type_id']==1 || $info['type_id']==3 || $info['type_id']==6){
        $sql  = "SELECT id,img FROM " . $GLOBALS['ecs']->table('release_img') ." where release_id=".$id;
        $info['img'] = $db->getAll($sql);
    }
...
function action_upload(){
    ...
    if(move_uploaded_file($tmp, $path.$image_name)){
            $imgId = '';
            if(!empty($id)){
                $imgId = insert_release_img($id, $path.$image_name);
            }
            $str = '';
            $str .= '<div class="xiaotu" id="del'.$tt.'">';
            //$str .= '<span><a href="javascript:;" onclick="del(a'.$tt.','.$id.','.$path.$image_name.')">[ - ]</a></span>';
            $str .= '<span><a href="javascript:;" onclick="del('.$tt.',\''.$imgId.'\',\''.$path.$image_name.'\')">[ - ]</a></span>';
            $str .=  '<img src="'.$path.$image_name.'"  class="preview">';
            $str .= '<input name="'.$file.'[]" value="'.$path.$image_name.'" type="hidden">';
            $str .= "</div>";
            echo $str;
    }else{
            echo '上传出错了！';
...
function action_release_view(){
    ...
    if($info['type_id']==1 || $info['type_id']==3 || $info['type_id']==6){
        ...
        $sql  = "SELECT id,img FROM " . $ecs->table('release_img') ." where release_id=".$id;
        $info['img'] = $db->getAll($sql);
    }
    //echo '<pre>';print_r($info);exit;
    $smarty->assign('info',$info);
    ...
    $smarty->assign('category', $category);
	$smarty->assign('type_id',$info['type_id']);
	//print_r($category);
 ...
 * @param string $img
 */
function insert_release_img($id,$img) {
    $sql = 'INSERT INTO ' .$GLOBALS['ecs']->table('release_img').
           ' (release_id,img)'.
            " VALUES ('$id', '$img')";
    $GLOBALS['db']->query($sql);
    return $GLOBALS['db']->insert_id();
...
function action_del_release(){
    $img  = trim($_POST['img']);
    $id = empty($_POST['id'])?'':intval($_POST['id']);
    if(!empty($id)){
        $GLOBALS['db']->query("delete from " . $GLOBALS['ecs']->table('release_img') . " where id='$id'");
    }
