...
function action_allot ()
{
	...
	/* 获取权限的分组数据 */
	$sql_query = "SELECT action_id, parent_id, action_code,relevance FROM " . $ecs->table('supplier_admin_action') . " WHERE parent_id = 0 and is_show=1";
	...
	/* 按权限组查询底级的权限名称 */
	$sql = "SELECT action_id, parent_id, action_code,relevance FROM " . $ecs->table('supplier_admin_action') . " WHERE is_show=1 and parent_id " . db_create_in(array_keys($priv_arr));
	$result = $db->query($sql);
