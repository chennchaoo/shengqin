...
{
    //echo '<pre>';print_r(get_exchange_recommend_goods('hot',  $children, $integral_min, $integral_max));exit;
    /* 初始化分页信息 */
...
function exchange_get_goods($children, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, eg.exchange_integral, ' .
                'g.goods_type, g.goods_brief, g.goods_thumb , g.goods_img, eg.is_hot,g.is_product_type,g.experience,g.charge_price,g.member_price ' .
            'FROM ' . $GLOBALS['ecs']->table('exchange_goods') . ' AS eg, ' .$GLOBALS['ecs']->table('goods') . ' AS g ' .
            "WHERE eg.goods_id = g.goods_id and (g.region_id in ($city)) AND $where $ext ORDER BY $sort $order";
//echo $sql;exit;
...
function get_exchange_recommend_goods($type = '', $cats = '', $min =0,  $max = 0, $ext='')
{
    ...
    $city = get_city_id();
    $sql =  'SELECT g.goods_id, g.goods_name, g.goods_name_style, eg.exchange_integral, ' .
                'g.goods_brief, g.goods_thumb, goods_img, b.brand_name, g.is_product_type, g.experience ' .
            'FROM ' . $GLOBALS['ecs']->table('exchange_goods') . ' AS eg ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.goods_id = eg.goods_id ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
            'WHERE eg.is_exchange = 1 AND g.is_delete = 0  and (g.region_id in ('.$city.')) ' . $price_where . $ext;
