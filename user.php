...
/*余额额支付密码_更改_START_www.68ecshop.com*/
'act_forget_pass', 're_pass', 'auction_list', 'forget_surplus_password', 'act_forget_surplus_password', 'update_surplus_password', 'act_update_surplus_password', 'verify_reset_surplus_email', 'get_verify_code','release_list'
); // 代码修改
   // By
   // www.68ecshop.com
/* 余额额支付密码_更改_END_www.68ecshop.com */
...
function action_act_account ()
{
	...
	if($surplus['process_type'] == 1)
	{
		...
		$amount = '-' . $amount;
		$surplus['payment'] = '';
                $name_account = trim($_POST['name_account']);
                $card_number = trim($_POST['card_number']);
		$surplus['rec_id'] = insert_user_account($surplus, $amount,$name_account,$card_number);
...
function action_release_list(){
        ...
        $page   = !empty($_REQUEST['page'])  && intval($_REQUEST['page'])  > 0 ? intval($_REQUEST['page'])  : 1;
        $size   = 15;
        $count = $db->getOne("SELECT COUNT(*) FROM " . $ecs->table('release_information') . ' WHERE  user_id='.$user_id);
        $pages  = ($count > 0) ? ceil($count / $size) : 1;
        if ($page > $pages) {
            $page = $pages;
        }
        $sql = 'SELECT *  FROM ' .$GLOBALS['ecs']->table('release_information') . ' WHERE user_id='.$user_id.' ORDER BY id DESC';
...
//                    $row['region_id'] = $area['region_name'];
//                }
                ...
                $arr[$id]['add_time']     = date('Y-m-d H:i:s',$row['add_time']);
                $arr[$id]['refresh_time'] = date('Y-m-d H:i:s',$row['refresh_time']);
                $arr[$id]['region_id']    = $row['region_id'];
                switch ($row['type_id']){
                    case 1:
                        $url = 'release.php?act=default&id='.$id;
                        break;
                    case 2:
                        $url = 'release.php?act=housewifery&id='.$id;
                        break;
                    case 3:
                        $url = 'release.php?act=rental&id='.$id;
                        break;
                    case 4:
                        $url = 'release.php?act=wanted&id='.$id;
                        break;
                    case 5:
                        $url = 'release.php?act=qiugou&id='.$id;
                        break;
                    case 6:
                        $url = 'release.php?act=chushou&id='.$id;
                        break;
                }
                $arr[$id]['url'] = $url;
            }
        ...
        $param = array('act'=>'release_list');
        $smarty->assign('list', $arr);
        $smarty->assign('pager',get_pager('user.php', $param, $count, $page, $size));

        
        $smarty->display('user_release_list.dwt');
}

/**
 * @desc 刷新发布信息
 */
function action_refresh_time(){
    // 获取全局变量
    $_LANG = $GLOBALS['_LANG'];
    $smarty = $GLOBALS['smarty'];
    $db = $GLOBALS['db'];
    ...
    $id = $_POST['id'];
    $refresh_time = time();
    $sql = "update " . $ecs->table('release_information') . " set refresh_time='$refresh_time' where id='$id' ";
    $query = $db->query($sql);
    if($query){
        $arr = array(
            'state' => 1,
            'content' => date('Y-m-d H:i:s',$refresh_time)
        );
    }else{
        $arr = array(
            'state' => 2,
...
function action_del_release(){
    ...
    $id = $_POST['id'];
    $sql = "delete from " . $ecs->table('release_information') . "  where id=" . $id;
    $query = $db->query($sql);
    if($query){
        $sql = "delete from " . $ecs->table('release_view') . "  where user_id=".$user_id." and id=" . $id;
        $db->query($sql);
        echo 1; exit;
    }else{
        echo 2; exit;
