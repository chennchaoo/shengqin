...
function action_send_mobile_code ()
{
	...
	$mobile_code = rand_number(6);
        $messages = "您的验证码是：" . $mobile_code . "，请不要把验证码泄露给其他人，如非本人操作，可不用理会";
        //$message = iconv("UTF-8","GBK",$messages);
        //$send_result = 'http://api.momingsms.com/?action=send&username=70212330&password=7f530124fbefc8c9f9f9a8dd369d8ba9&phone='.$this->mobile.'&content='.$message.'';
        //$result = curl_get($send_result);
        $content = $messages;
