...
//echo 'The website is under construction; please contact us by <a href="http://www.sdnawang.com">sdnawang</a>';exit;
define('IN_ECS', true);
...
//echo '<pre>';print_r(get_recommend_goods('hot'));exit;
if ((DEBUG_MODE & 2) != 2)
{
    $smarty->caching = true;
}
...
/*------------------------------------------------------ */
/* 缓存编号 */
//$smarty->assign('city_name',$_COOKIE['city_name']);  // 当前位置

$cache_id = sprintf('%X', crc32($_SESSION['user_rank'] .'-'.$_CFG['lang']));

//五楼
$smarty->assign('gongqiu',  get_release_information(3, $num=20));//房源供求
if (!$smarty->is_cached('index.dwt', $cache_id))
{
	...
	$sql = "SELECT value FROM " .$ecs->table("shop_config") ." WHERE id = 109";
    $qq = $db->GetOne($sql);
    if($qq){
        $qq = explode(',', $qq);
        //echo '<pre>';print_r($qq[0]);exit;
	...
	$smarty->assign('top_goods4',        get_top10(93));   
	 ///llx添加end
	 //一楼产品商城
    $smarty->assign('productCate',get_category(3,6));
    $smarty->assign('life_product', get_cat_recommend_goods('new', get_children(3), 6));//promote
	//print_r(get_cat_recommend_goods('new', get_children(3), 6));
	//二楼
    $smarty->assign('jiazhengCate',get_category(1,6));
    $smarty->assign('jiazhenggoods', get_cat_recommend_goods('new', get_children(1), 6));//hot
    //三楼
    $smarty->assign('pension',get_category(2,6));
    $smarty->assign('hot_pension',get_cat_recommend_goods('new', get_children(2), 6));//hot
    //四楼
    $smarty->assign('agent',get_category(6,6));
    $smarty->assign('agent_goods',get_cat_recommend_goods('new', get_children(6), 6));//promote
    
    //六楼
    $smarty->assign('jiazheng',  get_release_information(2,10));//找家政
    $smarty->assign('zuojiazheng',  get_release_information(1, $num=10));//做家政
	...
	$smarty->assign('brand_list',  get_brands_list());
	...
	$smarty->assign('notice',get_cat_articles($cat_id=12, 1, 20));
